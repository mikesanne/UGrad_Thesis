...
%     K = [572.87952, 0, 0; 0, 572.87952, 0; 390.26123, 248.79961, 1];
    ...
    I2 = rgb2gray(readimage(images,frame_step+1));
    ...
    %% Loop
    k = 1;
for i = 1+(frame_step*2):frame_step:numel(images.Files)
    i
    close all;
    rng('default');
    ...
    tGT = [xVicon(i)-xVicon(i-k*frame_step) yVicon(i)-yVicon(i-k*frame_step) zVicon(i)-zVicon(i-k*frame_step)];
    t2GT = [xVicon(frame_step+1)-xVicon(1) yVicon(frame_step+1)-yVicon(1) zVicon(frame_step+1)-zVicon(1)];
    scale = norm(tGT);%/norm(t2GT);
    k = 1;
    scale_vec = [scale_vec, scale]; 
