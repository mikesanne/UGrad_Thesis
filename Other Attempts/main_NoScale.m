function v = main_NoScale(start, finish, step)  
...
detector = cv.FeatureDetector('SIFT');
    ...
    mpairs_12 = matchFeatures(feat1, feat2, 'Unique', true);
    mpoints1_12 = points1(mpairs_12(:,1));  mpoints2_12 = points2(mpairs_12(:,2));
    mpoints1_loc = struct2table(mpoints1_12); mpoints2_loc = struct2table(mpoints2_12);
    ...
    [orient_12,loc_12,vpf] = relativeCameraPose(E, cameraParams, mpoints1_loc, mpoints2_loc);
...
if vpf<=0.8, warning('Problem with camera pose estimate?'); end
    ...
    tGT = [xVicon(2)-xVicon(1) yVicon(2)-yVicon(1) zVicon(2)-zVicon(1)];
    [R2_12,t2_12] = cameraPoseToExtrinsics(orient_12, loc_12);
    P2_12 = cameraMatrix(cameraParams, R2_12, tGT);
...
%     end
    ...
    % Detect Features for Image 2
    points3 = detector.detect(I3);
    feat3 = extractor.compute(I3, points3);
    
    % Match Features between image 1 and image 2 - Putative matches
    mpairs_23 = matchFeatures(feat2, feat3, 'Unique', true);
    mpoints2_23 = points2(mpairs_23(:,1));  mpoints3_23 = points3(mpairs_23(:,2));
    mpoints1_loc = struct2table(mpoints2_23); mpoints2_loc = struct2table(mpoints3_23);
    mpoints1_loc = mpoints1_loc{:,1}; mpoints2_loc = mpoints2_loc{:,1};
    %fh = figure; showMatchedFeatures(I1, I2, mpoints1_loc, mpoints2_loc, 'falsecolor'); 
...
if vpf<=0.8, warning('Problem with camera pose estimate?'); end
    ...
    [R3_23,t3_23] = cameraPoseToExtrinsics(orient, loc);
...
%     end
    tGT = [xVicon(i)-xVicon(i-1) yVicon(i)-yVicon(i-1) zVicon(i)-zVicon(i-1)];
    t2GT = [xVicon(2)-xVicon(1) yVicon(2)-yVicon(1) zVicon(2)-zVicon(1)];
...
%     end
    ...
    R3_23 = double(scale*R3_23*R2_23);
    P3_23 = cameraMatrix(cameraParams, R3_23, t3_23);
